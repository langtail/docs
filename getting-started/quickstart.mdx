...
<Steps>
  ...
  <Step title="Integrate with the sample app">
    ...
    1. Clone the quickstart repo using this command:
        ```
        git clone https://github.com/langtail/quickstart.git
        ```
    1. Open a terminal in the repo and install the packages using this command:
        ```
        npm install
        ```
    1. Duplicate the `.env.local.example` file and rename it to `.env.local`.
    1. Replace the following values in that file:
        - `LANGTAIL_API_KEY`: The project API key generated in the last step.
        - `LANGTAIL_PROMPT_SLUG`: The slug of the prompt. It should be `quickstart`.
        - `LANGTAIL_ENV`: The environment to invoke. It should be `staging`.
