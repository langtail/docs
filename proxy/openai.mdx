...
If you've already started building your application using the OpenAI SDK but want to start experimenting with Langtail, you can configure your existing code to start sending requests to the Langtail's proxy.

In this guide, we'll generate a Langtail API key, modify your existing code to send LLM requests via the Langtail's proxy, and then use Langtail's observability tools to see the requests coming from your app.
